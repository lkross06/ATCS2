...
__redis_server = redis.Redis(host="localhost", port=__REDIS_PORT, decode_responses=True) #token : profile json as string
...
def account_create(username:str, password:str) -> Profile:
    ...
    new_path = os.path.join("files", username, "")
...
:return: true if successful, false otherwise
'''
def profile_update(token:str = None, username:str = None, fname:str = None, lname:str = None, avatar:str = None) -> bool:
    try:
        #update sql
        cur = __sqlite3_conn.cursor()
        ...
        #update token if its an active account
        if token != None:
            ...
            if profile != None:
                ...
                __redis_server.set(token, str(profile))

                return True
            else:
                return False
        else: #if its imported and not in redis
            return True
    except:
        return False
...
'''
def profile_get(username:str) -> Profile | None:
...
'''
sets a token:profile pair into the redis database

:param profile: profile object to store

:return: string uuiv4 token
'''
def session_set(profile:Profile) -> str:
...
'''
deletes a token:profile pair from the redis database

:param token: token of pair to delete

:return: true if successful, false otherwise
'''
def session_delete(token:str) -> bool:
    try:
        __redis_server.delete(token)
        return True
    except:
        return False

'''
gets an active profile given the token

:param token: token of profile to get

:return: profile if found, None if invalid token/profile
'''
def session_get(token:str) -> Profile | None:
    try:
        profile = __redis_server.get(token)
        info = json.loads(profile)

        rtn = Profile()
        rtn.username = info.get("username")
        rtn.fname = info.get("fname")
        rtn.lname = info.get("lname")
        rtn.avatar = info.get("avatar")
        rtn.files = info.get("files")
...
'''
retrieves a list of the IPv4 addresses with active tcp/udp connections in redis

:return: list of active IPv4 addresses
'''
def connections_get():
    connections = __redis_server.lrange("connections", 0, -1)
    rtn = []

    for ip in connections:
        if __check_ipv4_addr(ip):
            rtn.append(ip)
