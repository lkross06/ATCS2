...
'''
class PlayFair:
    def __init__(self, password:str = None, size:int = 5):
        ...
        if password == None or password.get("password") == None:
            #arbitrary auto-generated password of random printable characters that is 1-10 characters long
            password = "".join([chr(x) for x in random.sample(sq_range, random.randint(1, 10))])
        else:
        ...
        try:
            if mode == "encrypt":
                ...
                with open(i, "rb") as file:
                    return file.readlines()[0].decode()
